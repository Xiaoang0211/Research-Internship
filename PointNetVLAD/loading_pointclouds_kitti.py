...
dict1 = "./KITTI_all/positive_sequence_D-3_T-30.json"
f = open(dict1, "r")
DICT_1 = json.load(f)
...
def cart2cyl(pointcloud, num_points):
    pointcloud_cyl = np.zeros((num_points,3))
    for idx, point in enumerate(pointcloud):
        rho = math.sqrt(point[0]**2 + point[1]**2)
        phi = math.atan(point[1]/point[0])
        z = point[2]
        pointcloud_cyl[idx,:] = [rho, phi, z]        
    return pointcloud_cyl
...
def load_pc_file(filename):
    ...
    pc = pc[ind]
    ind = np.argwhere(pc[:, 2] >= -1.2).reshape(-1)
    pc = pc[ind]
    # sample to 4096
    if pc.shape[0] >= NUM_POINTS:
        ind = np.random.choice(pc.shape[0], NUM_POINTS, replace=False)
        pc = pc[ind, :]
    else:
        ind = np.random.choice(pc.shape[0], NUM_POINTS, replace=True)
        pc = pc[ind, :]
...
def load_pc_label(filename):
    # returns Nx3 matrix
    pc = np.fromfile(filename, dtype=np.float32).reshape(-1,4) # xyz
    # # todo fov 100
    # pc = fov100(pc)
    # # # todo random drop30    drop90
    # pc = random_drop(pc, 30)
    ...
    # extract the labels of the point cloud
    pc_labels = pc[:,3]
    return pc_labels

def load_pc_labels(filenames):
    pcs_labels=[]
    for filename in filenames:
        #print(filename)
        pc_labels=load_pc_label(filename)
        assert pc_labels.shape[0]== NUM_POINTS
        pcs_labels.append(pc_labels)
    pcs_labels=np.array(pcs_labels)
    return pcs_labels
