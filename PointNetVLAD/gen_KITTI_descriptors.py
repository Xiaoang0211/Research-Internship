...
EVAL_BATCH_SIZE = 1
NUM_POINTS = 4096 # this must be the same integer as the NUM_POINTS from file loading_pointclouds_kitti.py
...
def get_global_descriptors():
    sequences = ["00", "02", "05", "06", "07", "08"] # test sequences
    for sq in sequences:
        LOG_DIR = LOG_DIR = 'log_fold{}/'.format(sq)
        RESULTS_FOLDER=LOG_DIR
        if not os.path.exists(RESULTS_FOLDER): os.mkdir(RESULTS_FOLDER)

        OUT_FEATURE_FOLDER = os.path.join(RESULTS_FOLDER, "feature_database/queries_gt_4096_completed") # where you would like to put you global descriptors
        ...
        with tf.Graph().as_default():
            ...
            ops = {'query': query,
                ...
                'pos_vecs': pos_vecs,
                'neg_vecs': neg_vecs}
            revisited_idx = list(QUERY_SETS[sq].keys())
            revisited_idx = [int(x) for x in revisited_idx]
            sq_dir = os.path.join(KITTI_submap_dir, sq)
            t1 = time.time()
            feature_db = get_latent_vectors(sess, ops, sq_dir, revisited_idx)
            t2 = time.time()
            print("time: ", t2 - t1)
...
def get_latent_vectors(sess, ops, sq_dir, revisited):
    train_file_idxs = []
    listDir(sq_dir, train_file_idxs)
    train_file_idxs.sort()
    train_file_idxs = [train_file_idxs[i] for i in revisited]
    is_training=False
